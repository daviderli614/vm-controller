...
func (a *StaticAutoscaler) RunOnce(currentTime time.Time) errors.AutoscalerError {
	...
	var pod *apiv1.Pod
	unSchedulerPodCheck := true
	if !a.isNodeScalerUpThreshold(allNodes, originalScheduledPods) {
		klog.V(2).Info("no resource expansion is required")
	} else {
		if len(unschedulablePods) == 0 {
			pod = buildScalerUpPod()
			unschedulablePods = append(unschedulablePods, pod)
			klog.V(2).Info("the capacity expansion condition has reached the threshold")
		} else {
			unSchedulerPodCheck = false
		}
	}

	unschedulablePodsToHelp, scheduledPods, err := a.processors.PodListProcessor.Process(
		unSchedulerPodCheck, a.AutoscalingContext, unschedulablePods, scheduledPods, allNodes, readyNodes,
	...
	if a.ScaleDownEnabled {
		...
		klog.V(4).Infof("disableScaleDownForLoop:%v, ScaleDownDelayAfterAdd:%v, scaleDownUnneededTime:%v, ScaleDownDelayAfterFailure:%v, ScaleDownDelayAfterDelete:%v", a.processorCallbacks.disableScaleDownForLoop, a.ScaleDownDelayAfterAdd, a.ScaleDownUnneededTime, a.ScaleDownDelayAfterFailure, a.ScaleDownDelayAfterDelete)
		klog.V(4).Infof("calculateUnneededOnly:%v, scaleDownInCooldown:%v, nodeDeletionTracker:%v",calculateUnneededOnly, scaleDownInCooldown, scaleDown.nodeDeletionTracker.IsNonEmptyNodeDeleteInProgress())
		klog.V(4).Infof("lastScaleUpTime:%v, lastScaleDownFailTime:%v, lastScaleDownDeleteTime:%v", a.lastScaleUpTime, a.lastScaleDownFailTime, a.lastScaleDownDeleteTime)
...
func buildScalerUpPod() *apiv1.Pod{
	var resource apiv1.ResourceRequirements
	j := `{"requests": {"cpu":"1m", "memory": "1Mi"}}`
	json.Unmarshal([]byte(j), &resource)
	pod := &apiv1.Pod{
...
func (a *StaticAutoscaler) isNodeScalerUpThreshold(nodes []*apiv1.Node, pods []*apiv1.Pod) bool {
	var podCpu, podMem int64 = 0, 0
	var nodeCpu, nodeMem int64 = 0, 0
	var requestMax float64 = 0
	checkNode := make(map[string]bool)

	for _, node := range nodes {
		if _, found := node.ObjectMeta.Labels["node-role.kubernetes.io/master"]; found {
			continue
		}
		if strings.Contains(node.Name, "uec") {
			klog.V(1).Info("contains uec node, node name:", node.Name)
			continue
		}
	...
	for _, pod := range pods {
		if !checkNode[pod.Spec.NodeName] {
			continue
		}
		for _, container := range pod.Spec.Containers {
			if cpuValue, found := container.Resources.Requests[apiv1.ResourceCPU]; found {
				podCpu += cpuValue.MilliValue()
			}
			if memValue, found := container.Resources.Requests[apiv1.ResourceMemory]; found {
				podMem += memValue.Value()
			}
		}
	}
	cpu, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", float64(podCpu)/float64(nodeCpu)),64)
	mem, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", float64(podMem)/float64(nodeMem)),64)
	if cpu > mem {
		requestMax = cpu
	} else {
		requestMax = mem
	}
	klog.V(4).Info("---calculate scaler up---", podCpu, podMem, nodeCpu, nodeMem, cpu, mem, a.ScaleUpUtilizationThreshold)
	if requestMax < a.ScaleUpUtilizationThreshold {
		return false
	}
	return true
}
