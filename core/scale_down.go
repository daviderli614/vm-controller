...
func (sd *ScaleDown) UpdateUnneededNodes(
	...
	for _, node := range filteredNodesToCheck {
		...
		if !sd.isNodeScalerOuterThreshold(node.Name, allNodes, pods) {
			klog.V(4).Infof("other nodes are greater than the threshold, node:%s", node.Name)
			continue
		}
...
func (sd *ScaleDown) isNodeScalerOuterThreshold(nodeName string, nodes []*apiv1.Node, pods []*apiv1.Pod) bool {
	...
	for _, pod := range pods {
		if m[pod.Spec.NodeName] {
			continue
		}
		for _, of := range pod.OwnerReferences {
			if of.Kind == "DaemonSet" {
				isDaemonSet = true
				break
			}
		}
		if pod.Spec.NodeName == nodeName && isDaemonSet {
			continue
		}
	...
	klog.V(4).Info("---calculate scaler down---", podCpu, podMem, nodeCpu, nodeMem, cpu, mem, requestMax, sd.context.ScaleUpUtilizationThreshold)
	if requestMax >= sd.context.ScaleUpUtilizationThreshold {
		return false
	}
	return true
}
