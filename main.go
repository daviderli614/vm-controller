...
var (
	...
	scaleUpUtilizationThreshold = flag.Float64("scale-up-utilization-threshold", 0.85,
		"Sum of cpu or memory of all pods running on the node divided by node's corresponding allocatable resource, below which a node can be considered for scale up")
...
func createAutoscalingOptions() config.AutoscalingOptions {
	...
	return config.AutoscalingOptions{
		...
		ScaleUpUtilizationThreshold:         *scaleUpUtilizationThreshold,
...
func main() {
	...
	klog.V(1).Infof("VM Controller %s", version.ClusterAutoscalerVersion)
